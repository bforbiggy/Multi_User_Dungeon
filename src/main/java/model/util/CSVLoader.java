...
public class CSVLoader {
    private static final String PERIOD_REGEX = Pattern.quote(".");
    private static final String COMMA_REGEX = Pattern.quote(",");
    private static final String VERT_BAR_REGEX = Pattern.quote("|");
    ...
    public static Item stringToItem(String input) {
        String[] tokens = input.split(PERIOD_REGEX);
        // Item conversion
    ...
    public static Chest stringToChest(String invString) {
        ...
        for (String itemString : invString.split(COMMA_REGEX)) {
    ...
    public static NPC stringToNPC(String npcData, String invString) {
        ...
        // Construct NPC
        String[] tokens = npcData.split(COMMA_REGEX);
        Stats stats = new Stats(parseInt(tokens[2]), parseInt(tokens[3]), parseInt(tokens[4]));
        NPC npc = new NPC(tokens[0], tokens[1], stats, Boolean.parseBoolean(tokens[5]), inv);
        return npc;
    }
    ...
    public static void writeStringToTile(String input, Tile tile, ArrayList<Object> runningList) {
        ...
        for (String objectString : input.split(VERT_BAR_REGEX)) {
            Object obj = null;
    ...
    public static Player loadPlayer(String playerSavePath) {
        ...
        try {
            ...
            // Read in player data
            String[] playerTokens = scanner.nextLine().split(COMMA_REGEX);
            ...
            String[] equipTokens = scanner.nextLine().split(COMMA_REGEX);
            ...
            String[] tokens = scanner.nextLine().split(COMMA_REGEX);
            for (String token : tokens)
                inventory.addItem(stringToItem(token));

            // Read in all items
            while (scanner.hasNextLine()) {
                tokens = scanner.nextLine().split(COMMA_REGEX);
                for (String token : tokens)
                    inventory.addItem(stringToItem(token));
            }
    ...
    public static Map loadMap(Player playerToLoad, String mapSavePath) {
        ...
        try {
                ...
                // Room parsing
                    ...
                    for (int h = 0; h < height; h++) {
                        ...
                        for (int w = 0; w < width; w++) {
                            ...
                            // If tile has an exit, add it to exit list for connection
                            if (tile.content instanceof Exit) {
                                Exit exit = (Exit) tile.content;
            ...
            System.out.println("Pre-existing map not found, generating new map...");
            map = null;
        }
        return map;
    }
